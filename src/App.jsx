import './App.scss';
import 'bootstrap/dist/js/bootstrap.bundle';
import { BrowserRouter, Route, Routes, Navigate, useNavigate } from 'react-router-dom';
import Nav from './Components/Nav';
// import Home from './Components/home/Main';
import MainBooks from './Components/books/Main';
import MainCats from './Components/cats/Main';
import MainRez from './Components/rezervacijos/Main';
import RegisterPage from './Components/register/Main';
import { login, logout, authConfig } from './Functions/auth';
import { useState, useEffect} from 'react';
import axios from 'axios';

function App() {

  const [roleChange, setRoleChange] = useState(Date.now());
  const [name, setName] = useState ('');


 return (
      <BrowserRouter>
        <ShowNav roleChange={roleChange} />
        <Routes>
          {/* <Route path="/" element={<RequireAuth role="user" setName={setName}><Home /></RequireAuth>}></Route> */}
          <Route path="/login" element={<LoginPage setRoleChange={setRoleChange} setName={setName} />} />
          <Route path="/logout" element={<LogoutPage setRoleChange={setRoleChange} />} />
          <Route path="/books" element={<RequireAuth role="admin" setName={setName}><MainBooks /></RequireAuth>}></Route>
          <Route path="/cats" element={<RequireAuth role="admin" setName={setName}><MainCats /></RequireAuth>}></Route>
          <Route path="/rezervacijos" element={<RequireAuth role="user" setName={setName}><MainRez useris={name} /></RequireAuth>}></Route>
          <Route path="/register" element={<RegisterPage setRoleChange={setRoleChange} />} />
        </Routes>
      </BrowserRouter>
  );
}


function ShowNav({ roleChange}) {
  const [status, setStatus] = useState(1);
  useEffect(() => {
    axios.get('http://localhost:3003/login-check?role=admin', authConfig())
      .then(res => {
        setStatus(res.data.status);
      })
  }, [roleChange]);
  return <Nav status={status} />
}

function RequireAuth({ children, role, setName }) {
  const [view, setView] = useState(<h2>Please wait...</h2>);

  useEffect(() => {
    axios.get('http://localhost:3003/login-check?role=' + role, authConfig())
      .then(res => {
        if ('ok' === res.data.msg) {
          setView(children);
          setName(res.data.name);
        }
        else if (res.data.status === 2) {
          setView(<h2>Unauthorize...</h2>)
        }
        else {
          setView(<Navigate to="/login" replace />);
        }
      })

  }, [children, role, setName]);

  return view;
}


function LoginPage({ setRoleChange, setName }) {
  const navigate = useNavigate();
  const [user, setUser] = useState('');
  const [pass, setPass] = useState('');

  const doLogin = () => {
    axios.post('http://localhost:3003/login', { user, pass })
      .then(res => {
        setRoleChange(Date.now());
        if ('ok' === res.data.msg) {
          login(res.data.key);
          user === 'admin' ? navigate('/books', { replace: true }) : navigate('/rezervacijos', { replace: true });
          setName(res.data.name)
        }
      })
  }
  return (


    <div className="container">
      <div className="row justify-content-center">
        <div className="col col-lg-4 col-md-12">
          <div className="card m-4">
            <h5 className="card-header">Login</h5>
            <div className="card-body">
              <div className="mb-3">
                <label className="form-label">name</label>
                <input type="text" className="form-control" value={user} onChange={e => setUser(e.target.value)} />
              </div>
              <div className="mb-3">
                <label className="form-label">password</label>
                <input type="password" className="form-control" value={pass} onChange={e => setPass(e.target.value)} />
              </div>
              <button onClick={doLogin} type="button" className="btn btn-outline-success">Login</button>
            </div>
          </div>
        </div>
      </div>
    </div>

  );
}

function LogoutPage({ setRoleChange }) {
  useEffect(() => {
    logout();
    setRoleChange(Date.now());
  }, [setRoleChange]);

  return (
    <Navigate to="/login" replace />
  )
}

export default App;